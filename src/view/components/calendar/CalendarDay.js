...
class CalendarDay extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      ...
      newState = {
        ...newState,
        enabled: this.isDateAllowed(this.props.date, this.props.datesBlacklist, this.props.datesWhitelist),
      };
      doStateUpdate = true;
    }
  ...
  renderDots(marking) {
    ...
    validDots = marking.dots
      ...
      .map((dot, index) => {
        ...
        return (
          <View key={dot.key || formattedDate + index} style={[baseDotStyle, { backgroundColor }, markedDatesStyle]} />
        );
      });
  ...
  render() {
    ...
    if (enabled && selected) {
      ...
      _dayContainerStyle = [dayContainerStyle];
    }
    ...
    if (DayComponent) {
      ...
      day = (
        <TouchableOpacity onPress={onDateSelected.bind(this, date)} disabled={!enabled}>
          <View style={[styles.dateContainer, responsiveDateContainerStyle, _dateViewStyle, _dayContainerStyle]}>
            {showDayName && (
